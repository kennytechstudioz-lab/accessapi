...
router.post('/register', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const curr of defaultCurrencies) {
      // Set initial balance for USD/EUR to make it look active or let it start at 0
      const isBase = curr.code === baseCurrency;
      const initialBalance = isBase ? 1000 : 0; // Seeding 1000 in base currency to give some initial funds
...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Verify Password (bypassed per request)
